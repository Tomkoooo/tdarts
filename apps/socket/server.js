...
io.on("connection", (socket) => {
  ...
  socket.on("throw", (data) => {
    ...
    // Log arrow count if it's a checkout
    if (data.isCheckout && data.arrowCount) {
      console.log(`🏹 Checkout with arrow count: ${data.arrowCount} for player ${data.playerId}`);
    }
  ...
  socket.on("leg-complete", (data) => {
    ...
    // Log the completed leg with winner arrow count
    const lastCompletedLeg = state.completedLegs[state.completedLegs.length - 1];
    if (lastCompletedLeg && lastCompletedLeg.winnerArrowCount) {
      console.log(`🏹 Leg ${lastCompletedLeg.legNumber} completed with winner arrow count: ${lastCompletedLeg.winnerArrowCount}`);
    }
...
function updateMatchStateOnThrow(matchId, data) {
  ...
  // Add arrowCount to throwData if it's a checkout
  if (data.isCheckout && data.arrowCount) {
    throwData.arrowCount = data.arrowCount;
  }
  ...
  // If this is a checkout, store the arrow count for leg completion
  if (data.isCheckout && data.arrowCount) {
    currentState.currentLegData.winnerArrowCount = data.arrowCount;
  }
...
function completeLeg(matchId, data) {
  ...
  const completedLeg = {
    ...
    winnerArrowCount: currentState.currentLegData.winnerArrowCount || null,
  ...
  currentState.currentLegData = {
    ...
    winnerArrowCount: null, // Reset for next leg
  };
  matchStates.set(matchId, currentState);
  return currentState;
}
