...
export default function ClubLayout({
  ...
  return (
    <div className="relative min-h-screen overflow-x-clip bg-background club-layout-container">
      {landingPage && (
        <style jsx global>{`
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 10% 12%, var(--color-primary) 0%, transparent 34%), radial-gradient(circle at 88% 22%, color-mix(in srgb, var(--color-accent) 70%, transparent) 0%, transparent 28%)",
        }}
      />

      <div className={cn(
        "relative flex h-56 flex-col justify-end overflow-hidden pb-8 md:h-72",
        landingPage?.coverImage ? "" : "bg-linear-to-br from-primary/10 via-card/30 to-background"
      )}>
        {landingPage?.coverImage ? (
          ...
          <div className="absolute inset-0 z-0 bg-linear-to-br from-primary/20 via-primary/8 to-transparent" />
        )}
          ...
          <div className="rounded-3xl border border-white/15 bg-card/35 p-4 backdrop-blur-lg shadow-[0_18px_45px_rgba(0,0,0,0.35)] md:p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-muted-foreground" style={landingPage?.coverImage ? { color: "rgba(255,255,255,0.85)" } : {}}>
                  {t("subtitle")}
                </p>
              </div>
            </div>
      ...
      <div className="relative z-10 mx-auto -mt-8 w-full max-w-7xl px-4 pb-24 md:-mt-12 md:pb-10">
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4">
          <div className="sticky top-3 z-30 rounded-2xl border border-border/60 bg-card/85 p-2 shadow-[0_10px_30px_rgba(0,0,0,0.3)] backdrop-blur-xl">
            <TabsList className="flex h-auto w-full gap-1 overflow-x-auto rounded-xl bg-transparent p-0 sm:flex-wrap sm:justify-between [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {tabs.map((tab) => (
                <TabsTrigger
                  key={tab.key}
                  value={tab.key}
                  className="flex min-w-[66px] items-center justify-center gap-1 rounded-lg px-2 py-2 text-[11px] font-semibold text-muted-foreground transition-all duration-200 hover:bg-muted/25 hover:text-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground sm:min-w-[96px] sm:px-3 sm:text-sm"
                >
          ...
          <div className="rounded-3xl border border-border/60 bg-linear-to-br from-card/95 via-card/80 to-card/65 p-4 shadow-[0_20px_55px_rgba(0,0,0,0.35)] backdrop-blur-xl md:p-8 min-h-[400px]">
